  ...
  function renderNextOrSubmitBtn() {
    if (Children.count(children) == 3) {
      ...
      if (step === 0) {
        return showNext ? (
          <button
            className="bg-blue-500 hover:bg-blue-700 border border-blue-500 text-white font-bold py-2 px-4 rounded"
      ...
      <div className="md:flex flex-col w-56 pr-2">
        {Children.map(children, (child, idx) => {
          if (idx === step) {
            return (
              <div
                className="block font-bold pl-5 pt-3 pb-3 rounded-lg"
                style={{
                  backgroundImage:
                    "linear-gradient(to bottom, #D799D5, #B5B9F1)",
                }}
              >
                <div className="inline-block mr-2">
                  <span className="flex h-6 w-6 text-md items-center justify-center rounded-full border border-2 border-black">
                    {idx + 1}
                  </span>
                </div>
